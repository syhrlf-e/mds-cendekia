...
<template>
  <div class="min-h-screen bg-bg-base flex min-w-5xl">
    ...
    <aside class="hidden lg:flex flex-col w-64 bg-primary-800 text-white shrink-0 border-r border-primary-900/50 shadow-xl sticky top-0 h-screen overflow-y-auto">
      ...
      <nav class="grow py-6 px-4 flex flex-col gap-2">
        <NuxtLink
          v-for="item in menu"
          :key="item.name"
          :to="item.path"
    ...
    <main class="grow flex flex-col min-h-screen relative overflow-hidden max-w-full">
      <div class="grow p-8 bg-bg-base overflow-y-auto">
        <NuxtPage />
      </div>
    </main>
  </div>
