...
type StudentDto = Record<string, any>
...
const createDisplayNis = (item: StudentDto) => {
  ...
  const sourceId = normalizeText(item.kode_pendaftaran || item.nomor_pendaftaran || item.id || item.nisn)
...
const mapStudent = (item: StudentDto, fallbackFromPendaftar = false): Student | null => {
  ...
  const kodePendaftaran = normalizeText(item.kode_pendaftaran || item.nomor_pendaftaran || item.kode || item.id)
...
const loadStudents = async () => {
  ...
  const endpoints = [
    { url: '/api/siswa/data', fallbackFromPendaftar: false },
    { url: '/api/siswa', fallbackFromPendaftar: false },
    { url: '/api/pendaftar/data', fallbackFromPendaftar: true }
  ]
  ...
  students.value = []
  loadError.value = 'Data siswa belum bisa diambil dari server.'
  isLoading.value = false
}
