...
const handleSort = (key: 'nama' | 'tanggal') => {
  if (sortKey.value === key) {
    ...
    else sortKey.value = '' // reset if clicked 3rd time
  } else {
    sortKey.value = key
    sortOrder.value = 'asc'
  }
...
const handleView = (id: string) => {
  console.log('Lihat detail untuk:', id)
}
...
<template>
  <div class="flex flex-col h-full animate-in fade-in duration-300">
    ...
    <div class="bg-bg-surface border border-border rounded-2xl flex-grow flex flex-col shadow-sm overflow-hidden">
  ...
  <!-- Modal Detail Pendaftar (Phase 31) -->
  <AppModal v-model="isDetailModalOpen" width="w-full max-w-[80vw]" v-if="selectedItem">
    ...
    <!-- Modal Body (Layout with Sidebar) -->
    ...
    <!-- Sticky Footer -->
    <template #footer>
      <AppButton variant="danger" class="min-w-[120px]" @click="promptReject" v-if="selectedItem?.status === 'pending'">
        Tolak
      </AppButton>
      <AppButton variant="primary" class="min-w-[120px]" @click="promptApprove" v-if="selectedItem?.status === 'pending'">
        Terima
      </AppButton>
    ...
    <AppModal v-model="isApproveModalOpen" title="Konfirmasi Pendaftaran" width="max-w-md">
    ...
    <!-- Modal Reject (Phase 36) -->
    <AppModal :modelValue="isRejectModalOpen" @update:modelValue="attemptCancelReject" title="Tolak Pendaftar" width="max-w-lg">
    ...
    <!-- Modal Guard (Cancel Reject) -->
    <AppModal v-model="isRejectGuardOpen" title="Batalkan Penolakan?" width="max-w-sm">
